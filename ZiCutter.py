...
def loadHeZi(path, JiZi):
    ...
    values.sort()
    logger.info(f"  {path} --> loadHeZi {len(HeZi)}  values:{len(values)}")
    return HeZi, values
...
class ZiCutter:
    def __init__(self, dir=""):
        ...
        self.HeZiPath = os.path.join(self.here, "data/HeZi.txt")
        self.JiZiPath = os.path.join(self.here, "data/JiZi.txt")
    ...
    def load(self, dir=""):
        if dir:
            ...
            self.JiZiPath = os.path.join(dir, "JiZi.txt")
        HeZi, JiZi = loadHeZi(self.HeZiPath, self.vocab)

        self.HeZi = HeZi
        if os.path.exists(self.JiZiPath):
            JiZi += open(self.JiZiPath).read().splitlines()
        logger.info(f"ZiCutter load HeZi:{len(HeZi)} JiZi:{len(JiZi)}")
        self.vocab |= set(JiZi)
    ...
    def build(self, roots=[]):
        ...
        JiZi = list(self.vocab)+list(YuanZi)+roots
        # JiZi=[ x for x in JiZi if not odd(x) ]
        He2Zi.build(JiZi, ChaiZiPath=os.path.join(self.here, "data/ChaiZi.txt"), YiTiZiPath=os.path.join(self.here, "data/YiTiZi.txt"),
